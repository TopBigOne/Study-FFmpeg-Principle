...
int save_rgb_to_file(AVFrame *frame, int num) {
    char pic_name[200] = {0};
    sprintf(pic_name,
            "/Users/dev/Documents/Android_work/main_ffmpeg/FFmpeg-Principle/14_sws_scale-1/doc/rgba_8888_%d.yuv",
            num);
...
int my_code() {
    ...
    char *file_name = "/Users/dev/Desktop/mp4/柯柯柯啊-姑娘在遠方.mp4";
    ...
    for (;;) {
        ...
        ret = av_read_frame(fmt_ctx, avPacket);
        ...
        if (ret == AVERROR_EOF) {
            ...
            TAG_RETRY:
            if ((avcodec_send_packet(avctx, avPacket) == AVERROR(EAGAIN))) {
